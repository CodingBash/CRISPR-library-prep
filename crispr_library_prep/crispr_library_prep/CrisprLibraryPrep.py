...
class ExactHypergeom:
    ...
    def pmf(self, k):
        numerator = mpmath.log(mpmath.binomial(self.M, k)) + mpmath.log(mpmath.binomial(self.N - self.M, self.n - k))
        denominator = mpmath.log(mpmath.binomial(self.N, self.n))
        return float(mpmath.exp(numerator - denominator))
